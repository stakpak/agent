...
final class ScriptTab: Identifiable {
    ...
    /// Expanded state of the Steps (tool calls) disclosure inside the
    /// LLM Output HUD. Persisted across Cmd+B hide/show cycles and step
    /// count changes — without this, SwiftUI tears down ToolStepsView and
    /// resets its local @State, collapsing it every time a step is added
    /// or the HUD is toggled.
