    ...
    fn create_option_content(&self, option: &RecoveryOption) -> StyledLineContent {
            ...
            serde_json::from_value::<Vec<RecoveryAction>>(option.state_edits.clone())
        {
            if !actions.is_empty() {
                    ...
                    match action.recovery_operation {
                        RecoveryOperation::Truncate => {
                            full_content.push_str(&format!(
                                "- Truncate: Truncating conversation history after message {}\n",
                        ...
                        RecoveryOperation::RemoveTools => {
                            if let Some(ids) = action.failed_tool_call_ids_to_remove {
                                full_content.push_str(&format!(
                                    "- Remove: Removing failed tool calls: {}\n",
                                    ids.join(", ")
                                ));
                            }
                        }
                        RecoveryOperation::Append => {
                            if let Some(content) = action.content {
                                ...
                                let snippet = if content_str.len() > 60 {
                                    format!("{}...", &content_str[..60])
                                } else {
                                    content_str
                                };
                                full_content.push_str(&format!(
                                    "- Append: Appending new message: {}\n",
                                    snippet
                                ));
                            }
                        }
                ...
                full_content.push_str("\n# NOTE: These operations are irreversable!");
            }
        }
