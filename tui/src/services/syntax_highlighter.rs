...
pub fn apply_syntax_highlighting(text: &str, extension: Option<&str>) -> Vec<Line<'static>> {
    ...
    let extension = extension.or_else(|| Some("md"));
    ...
    for line in LinesWithEndings::from(text) {
        let ranges = highlighter.highlight_line(line, &syntax_set).unwrap_or_else(|_| vec![(syntect::highlighting::Style::default(), line)]);
