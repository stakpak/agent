...
pub async fn run_async(ctx: AppConfig, config: RunAsyncConfig) -> Result<AsyncOutcome, String> {
    ...
    // Count prior steps from checkpoint (assistant messages with tool_calls = steps)
    ...
    let should_add_prompt = if let Some(resume_input) = &config.resume_input {
        ...
        // (the prompt field is empty in that case anyway)
    ...
    loop {
        ...
        if let Some(tool_calls) = tool_calls {
            ...
            // This is a normal completion, not a pause. The checkpoint is saved
            // so the conversation can be resumed later via resume_subagent_task
            // if the parent agent wants to continue.
            print!(
                "{}",
