...
pub async fn handle_status(
    ...
    // API reachability check — only meaningful for the stakpak provider on the active profile.
    ...
    match active_stakpak_auth {
        Some(auth) => match probe_api(&config.api_endpoint, &auth).await {
            Ok(identity) => println!("API check:      ✓ reachable as {}", identity),
            Err(error) => println!("API check:      ✗ {}", error),
        },
        None => println!(
            "API check:      skipped (no stakpak credential on active profile '{}')",
            active_profile
        ),
    }
...
/// Print " key=...XXXX" for API keys, "" for OAuth (token suffixes are sensitive).
