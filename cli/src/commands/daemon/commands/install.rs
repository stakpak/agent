//! Daemon install command - install as a system service.

use super::service::{Platform, get_service_config_path, install_service, is_service_installed};

/// Install the daemon as a system service.
pub async fn install_daemon(force: bool) -> Result<(), String> {
    let platform = Platform::detect();

    println!("Installing stakpak daemon as a system service...");
    ...
    if !config_path.exists() {
        println!(
            "\x1b[33m⚠\x1b[0m  No daemon configuration found at: {}",
            config_path.display()
        );
        println!();
        println!("Create a configuration first with:");
        println!("  stakpak daemon init");
        println!();
        println!("Then run this command again.");
        return Err("Daemon configuration not found".to_string());
    }
