...
impl Sandbox {
    ...
    pub fn execute_command(&self, command: &str, args: &[&str]) -> Result<ExitStatus> {
        ...
        self.logger.log_command(command, allow_network, is_destructive);
        ...
        log::info!("Kernel capabilities - Landlock: {}, Seccomp: {}", has_landlock, has_seccomp);
        ...
        match cmd.status() {
            ...
            Err(e) => {
                self.logger.log_security_block("command_failed", &e.to_string());
