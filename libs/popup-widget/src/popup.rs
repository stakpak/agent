use crate::{
    PopupConfig, PopupState, PopupEvent, PopupEventResult, PopupContent, Alignment,
    tab::render_custom_tabs,
};
use ratatui::{
    Frame,
...
impl PopupWidget {
    ...
    pub fn render(&mut self, f: &mut Frame, terminal_size: Rect) {
        ...
        let popup_background = Block::default()
    ...
    fn render_with_tabs(&mut self, f: &mut Frame, content_area: Rect) {
        ...
        if self.config.show_title {
            if let Some(title) = &self.config.title {
                ...
                let title_style = self.config.title_style.add_modifier(ratatui::style::Modifier::BOLD);
        ...
        if let Some(footer_lines) = &self.config.footer {
            ...
            let footer_lines: Vec<Line> = footer_lines.iter()
    ...
    fn calculate_max_scroll(&self, terminal_size: Rect) -> usize {
        ...
        let available_height = if self.config.show_tabs && !self.config.tabs.is_empty() {
            ...
            if has_footer {
                let footer_height = self.config.footer.as_ref()
                    .map(|footer| footer.len())
                    .unwrap_or(0);
            ...
            eprintln!("DEBUG: Available height (with tabs) - content_area.height: {}, fixed_height: {}, available: {}", content_area.height, fixed_height, available);
            available
        } else {
        ...
        let content_height = if self.config.show_tabs && !self.config.tabs.is_empty() {
            if let Some(selected_tab) = self.config.tabs.get(self.state.selected_tab) {
                selected_tab.content.calculate_rendered_height(content_area.width)
            } else {
                0
            }
        } else if let Some(content) = &self.content {
            content.calculate_rendered_height(content_area.width)
        } else {
            0
        };
        
        // Debug: Print the scroll calculation details
        eprintln!("DEBUG: Scroll calculation - content_height: {}, available_height: {}", content_height, available_height);
        ...
        let max_scroll = if content_height > available_height {
            ...
            eprintln!("DEBUG: Max scroll calculated: {}", scroll);
            scroll
        } else {
            eprintln!("DEBUG: No scrolling needed");
            0
        };
