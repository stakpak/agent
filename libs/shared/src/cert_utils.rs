...
impl CertificateStrategy {
    ...
    /// For Ephemeral: generates a new chain in memory
    /// For Persistent: loads the chain from disk
    pub fn get_certificate_chain(&self) -> Result<CertificateChain> {
        match self {
            Self::Ephemeral => CertificateChain::generate(),
            Self::Persistent(_path) => {
                // For persistent, we need to load from disk
                ...
                // The server config is loaded separately via load_server_config()
