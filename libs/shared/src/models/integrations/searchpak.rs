...
pub async fn start_search_services() -> Result<SearchPakConfig, AgentError> {
    // 1. Try to load existing config
    if let Some(config) = load_search_services_config() {
        ...
        if health_check_api(&api_url).await.is_ok()
            && health_check_searxng(&searxng_url).await.is_ok()
        {
            return Ok(config);
        }
    ...
    let searxng_image =
        env::var("SEARXNG_IMAGE").unwrap_or_else(|_| "299shehab299/searchpak:searxng".to_string());
    let api_image =
        env::var("API_IMAGE").unwrap_or_else(|_| "299shehab299/searchpak:api".to_string());

    // Check if images exist locally
    ensure_image_exists(&searxng_image)?;
    ensure_image_exists(&api_image)?;

    let (searxng_port, api_port) = find_two_available_ports()?;
    ...
    let searxng_container_id =
        crate::container::run_container_detached(searxng_config).map_err(|e| {
            AgentError::BadRequest(BadRequestErrorMessage::ApiError(format!(
    ...
    let api_container_id = crate::container::run_container_detached(api_config).map_err(|e| {
        AgentError::BadRequest(BadRequestErrorMessage::ApiError(format!(
    ...
    save_search_services_config(&config)?;

    Ok(config)
}

pub async fn stop_search_services() -> Result<(), AgentError> {
    if let Some(config) = load_search_services_config() {
...
pub fn stop_search_services_sync() -> Result<(), AgentError> {
    if let Some(config) = load_search_services_config() {
        crate::container::stop_container(&config.searxng_container_id).map_err(|e| {
            AgentError::BadRequest(BadRequestErrorMessage::ApiError(format!(
                "Failed to stop SearXNG container: {}",
...
pub async fn check_search_services() -> Result<bool, AgentError> {
    if let Some(config) = load_search_services_config() {
        let api_url = format!("http://localhost:{}", config.api_port);
        let searxng_url = format!("http://localhost:{}", config.searxng_port);
...
pub fn load_search_services_config() -> Option<SearchPakConfig> {
...
pub fn save_search_services_config(config: &SearchPakConfig) -> Result<(), AgentError> {
...
pub async fn health_check_api(api_url: &str) -> Result<(), AgentError> {
...
pub async fn health_check_searxng(searxng_url: &str) -> Result<(), AgentError> {
...
#[derive(Debug)]
pub struct SearchServicesGuard;

impl Drop for SearchServicesGuard {
    fn drop(&mut self) {
        let _ = stop_search_services_sync();
    }
}
