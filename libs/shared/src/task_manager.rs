...
pub struct Task {
    ...
    /// Information stored when a task pauses (exit code 10).
    pub pause_info: Option<PauseInfo>,
}

/// Information stored when a task pauses.
#[derive(Debug, Clone, serde::Serialize)]
pub struct PauseInfo {
    /// The checkpoint ID extracted from the task's output.
    pub checkpoint_id: Option<String>,
    /// The raw JSON output from the paused process.
...
pub enum TaskError {
    ...
    #[error("Task not resumable (not paused or completed): {0}")]
        ...
        match msg {
            ...
            TaskMessage::TaskUpdate { id, completion } => {
                if let Some(entry) = self.tasks.get_mut(&id) {
                    ...
                    // Both states may need resuming (paused for approval/input,
                    // completed subagents that the parent wants to continue)
                    if matches!(
                        completion.final_status,
                        TaskStatus::Paused | TaskStatus::Completed
                    ) {
                        // Try to parse checkpoint_id from the output (JSON with checkpoint_id field)
                        let checkpoint_id =
