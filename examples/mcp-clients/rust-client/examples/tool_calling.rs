...
use rig::{
    ...
    completion::Prompt,
    providers::gemini::{self, completion::GEMINI_2_5_FLASH},
};
use rmcp::{
...
async fn main() -> Result<()> {
    ...
    info!("Setting up LLM agent with automatic tool calling");
    let api_key =
        std::env::var("GEMINI_API_KEY").expect("`GEMINI_API_KEY` environment variable must be set");
    let gemini_client = gemini::Client::new(&api_key)?;
    let agent = gemini_client
        .agent(GEMINI_2_5_FLASH)
        .preamble("You are a helpful devops assistant with access to tools related to devops")
        .rmcp_tools(tools, service.peer().to_owned())
        .build();

    info!("LLM agent initialized with automatic tool calling");
    let response = agent
        .prompt("generate a password with length of 20 and add symbols")
        .await?;
    info!("LLM response:");
    info!("{}", response);
